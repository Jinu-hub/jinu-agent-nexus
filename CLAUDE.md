...
| Market panel (sidebar) | `src/panels/MarketPanel.tsx` + `ReportReader.tsx` (wide modal + ## TOC + Topics section chips) — briefs/today + audio/today + reports/today (lazy; Topics reuses) + Ask in chat chips (`src/lib/market-suggestions.ts`); off-switches `SHOW_REPORT_TOPIC_CHIPS` / `SHOW_TOPICS_SECTION` in `ReportReader.tsx`; wired in `App.tsx` |

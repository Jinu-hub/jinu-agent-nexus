...
DAMI warm paper (cream + orange accent) is global — shell and report pages share `src/index.css` tokens.

```
┌────────────┬──────────────────────────┬──────────────────────────┐
│  Ask rail  │  DAMI chat               │  [Market][Memory]…       │
...
| Report "For you" | `POST /api/market/for-you` | `worker/market-for-you.ts` + `for_you_summaries` in `worker/my-memory.ts` | Saved interests ∩ report keys by **target** (Tag OpenAI ↔ Company OpenAI); passages ≤5 → grounded LLM (What/Detail/Why); cached on (item, lang, interest-set + prompt-version hash) |
...
worker/index.ts          HTTP entry — routes only, thin
    ...
    ├── POST /api/market-vector/cron/run → daily ingest Cron once
...
| `src/chat/Chat.tsx` | Shell chat — header report exits, `ChromePrefs`, DAMI intro empty state, helper/panel drawer toggles, composes `ChatParts` |

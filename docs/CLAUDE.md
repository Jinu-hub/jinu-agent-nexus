...
| Agent / MyMemory instance | `src/lib/agent-identity.ts` + `use-agent-instance.ts` + `src/lib/auth.tsx` — Phase 1 `guest_*`; Phase 2 Supabase user id; Phase 3 `isAdmin` via Worker `ADMIN_*`; Phase 4 global panel defaults on ChatAgent `"default"` (`/api/panel-defaults`). **`topic_labels` always `"default"`.** [`INSTANCE_DATA.md`](./INSTANCE_DATA.md) |

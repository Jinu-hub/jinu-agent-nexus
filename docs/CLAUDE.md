...
| UI chat shell | `src/chat/Chat.tsx`, `Message.tsx`, `Markdown.tsx` + `HomeReportExits.tsx` — empty state = LYRA intro + report landing cards (`REPORT_PAGES`) + CF stack + `HOME_CHAT_SUGGESTIONS`; header links leave the shell for `/<slug>` (`xl+`); Reset/Clear labels `xl+`; right panels dock at `lg+`, drawer toggle below (`App.tsx` `panelOpen`) |

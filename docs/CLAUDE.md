...
| UI chat shell | `src/chat/Chat.tsx`, `Message.tsx`, `Markdown.tsx` + `HomeReportExits.tsx` + `ChatHelperRail.tsx` — empty state = LYRA intro + how-to + report landing cards (`REPORT_NAV_CATEGORIES`); helper rail = Topics (collapsible, default open) + Ask fills leftover (`reportPageSuggestions` one-row list + Ask-only scroll); header category menus (`Market` ▾ → pages, always shown); header `ChromePrefs` (theme + `content_lang`); Reset/Clear labels `xl+`; helper rail docks at `xl+`, right panels at `lg+`, drawers below (`App.tsx` `helperOpen` / `panelOpen`) |

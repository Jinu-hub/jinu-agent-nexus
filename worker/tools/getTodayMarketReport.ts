...
export function createGetTodayMarketReportTool(agent: ChatAgent, env: Env) {
  ...
  return tool({
    description:
      `Fetch Market Memory full report grounding (item_contents via brief.target_id). Use for 풀리포트 / digest / highlight questions — NOT a full reprint. Language = Settings content_lang. Dates: Asia/Seoul today=${today}, expected latest (omit date)=${latest}, yesterday=${yesterday}. After the tool returns: answer briefly from title/summary/excerpt/highlights; do NOT paste the full content; point to Market tab → Report.`,

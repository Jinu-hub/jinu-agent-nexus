...
| Market panel | Market tab | `src/panels/MarketPanel.tsx`, `ReportReader.tsx`, `src/lib/market-fetch.ts`, `use-market-day-data.ts`, `use-market-preferences.ts`, `market-suggestions.ts` | `/api/market/day` + per-series tabs + wide reader; Topics/Ask on home helper rail; day/prefs fetch shared with that rail |
